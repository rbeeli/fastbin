...
@inline function child1!(obj::Parent, value::ChildFixed)
    @assert binary_size(value) > 0 "Cannot set field `child1`, parameter struct of type `ChildFixed` not finalized. Call fastbin_finalize!(obj) on struct after creation."
...
@inline function child2!(obj::Parent, value::ChildVar)
    @assert binary_size(value) > 0 "Cannot set field `child2`, parameter struct of type `ChildVar` not finalized. Call fastbin_finalize!(obj) on struct after creation."
