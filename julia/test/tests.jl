...
@testitem "StreamTrade (getter/setter functions)" begin
    using StringViews
    ...
    @assert fastbin_calc_binary_size(
        StreamTrade, StringView("MNTUSDT"), StringView("239f859d-d32b-577e-90bc-b1b0dbbb06e0")
    ) == fastbin_binary_size(t)

    show(t)
end

@testitem "StreamTrade (getter/setter properties)" begin
    using StringViews
    using fastbin.my_models
    ...
    t.server_time = 1708572597106 * 1_000_000
    t.recv_time = 1708572597108 * 1_000_000
    t.symbol = "MNTUSDT"
    t.fill_time = 1708572597104 * 1_000_000
    t.side = TradeSide.Sell
    t.size = 1.0
    t.price = 0.70950
    t.price_chg_dir = TickDirection.PlusTick
    t.trade_id = "239f859d-d32b-577e-90bc-b1b0dbbb06e0"
    t.block_trade = false

    fastbin_finalize!(t)

    @assert t.server_time == 1708572597106 * 1_000_000
    @assert t.recv_time == 1708572597108 * 1_000_000
    @assert t.symbol == "MNTUSDT"
    @assert t.fill_time == 1708572597104 * 1_000_000
    @assert t.side == TradeSide.Sell
    @assert t.size == 1.0
    @assert t.price == 0.70950
    @assert t.price_chg_dir == TickDirection.PlusTick
    @assert t.trade_id == "239f859d-d32b-577e-90bc-b1b0dbbb06e0"
    @assert t.block_trade == false
...
@testitem "VectorOfFixedSizedStructs" begin
    ...
    @assert fastbin_binary_size(v) == fastbin_calc_binary_size(v)
    @assert fastbin_calc_binary_size(VectorOfFixedSizedStructs, vals, str_) ==
        fastbin_binary_size(v)
end
...
@testitem "Parent round-trip (nested var/fixed)" begin
    ...
    @test fastbin_binary_size(p) == fastbin_calc_binary_size(Parent, child2(p), str(p))
end
